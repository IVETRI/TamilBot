...
async def _(event):
    ...
    #packname = f"TamilStickers"
    #packshortname = f"TamilBot_{userid}_ns"  # format: Uni_Borg_userid
    if userid == 1492186775:
        packname = f"TamilStickers"
        packshortname = "Tamil"
    elif userid ==1169076058:
        packname = f"@TamilSupport's fav pack"
        packshortname = "Tamil"
    else:
        packname = f"TamilBot{userid}"
        packshortname = f"TamilBot_{userid}_ns"
    await event.edit("`Look way,it's a gurl!\nMeanwhile, lemme kang this stcker over hehe ヽ༼ ಠ益ಠ ༽ﾉ`")

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "TamilBot_Sticker.png"
    ...
    if is_a_s:
        ...
        if userid == 948408212:
            ...
            packshortname = f"TamilBot_{userid}_an" # format: Uni_Borg_userid
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    pack += 1
                    prevv = int(pack) - 1
                    packname = f"{user.first_name}'s TamillBot Vol.{pack}"
                    packshortname = f"Vol_{pack}_with_{user.first_name}"
                    #if userid == 1492186775:
                       # packname = f"{user.first_name}'s TamilBot Vol.{pack}"
                       # packshortname = "Vol. Tamilbot"
                   # else:
                       # packname = f"Vol._{pack}_TamilBot{userid}"
                        #packshortname = f"Vol._{pack}_TamilBot_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
