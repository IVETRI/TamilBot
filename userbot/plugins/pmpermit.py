    ...
    async def approve_p_m(event):
        ...
        if event.is_private:
            if chat.id == 1492186775 or chat.id == 1169076058:
              await event.edit("மன்னிக்கவும், எனது எஜமானரை நான் ஏற்க முடியாது")
            else:  
              if pmpermit_sql.is_approved(chat.id):
                  pmpermit_sql.disapprove(chat.id)
                  await event.edit("தனிப் பதிவிற்கு ஒப்புதல் அளிக்கப்படவில்லை [{}](tg://user?id={})".format(firstname, chat.id))
    ...
    if event.is_private:
        if not pmpermit_sql.is_approved(chat.id):
          if chat.id in PREV_REPLY_MESSAGE:
            await PREV_REPLY_MESSAGE[chat.id].delete()
            del PREV_REPLY_MESSAGE[chat.id]
          pmpermit_sql.approve(chat.id, "என் எஜமான்🙈🙈")
          await borg.send_message(chat, "என் எஜமான் வந்துவிட்டார் .... நன்றி எஜமான்")
